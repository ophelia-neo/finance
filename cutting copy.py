...
for expected, actual in column_mapping.items():
    if actual in df.columns:
        ...
        if expected == 'Net_Profit_Margin':
            # Option 1: Try to find a similar column
            ...
            if possible_profit_cols:
                ...
                # Option 2: Create a dummy column with zeros (all companies will pass this filter)
...
# Dictionary to store all filtered dataframes
step_dataframes = {}
step_dataframes['Original_Data'] = df.copy()

# Apply filters in the exact order you requested
steps = []
...
if column_mapping['Debt_to_Equity'] in df.columns:
    steps.append(("Step6_DebtToEquity", df[column_mapping['Debt_to_Equity']].between(1, 2)))

if column_mapping['Interest_Coverage'] in df.columns:
    steps.append(("Step7_InterestCoverage", df[column_mapping['Interest_Coverage']] >= 3))

if column_mapping['Quick_Ratio'] in df.columns:
    steps.append(("Step8_QuickRatio", df[column_mapping['Quick_Ratio']] >= 1))

if column_mapping['Current_Ratio'] in df.columns:
    steps.append(("Step9_CurrentRatio", df[column_mapping['Current_Ratio']].between(1.5, 2.5)))


# Apply filters and store each step
df_steps = df.copy()
for step_name, condition in steps:
    ...
    # Store the dataframe for this step
...
try:
    with pd.ExcelWriter(final_workbook, engine='openpyxl') as writer:
        ...
        # Write each step to a separate sheet
        for sheet_name, df_step in step_dataframes.items():
            print(f"💾 Writing sheet: {sheet_name} ({len(df_step)} rows)")
            
            # Excel sheet names have a 31 character limit
            sheet_name_clean = sheet_name[:31] if len(sheet_name) > 31 else sheet_name
            
            # Write dataframe to sheet
            df_step.to_excel(writer, sheet_name=sheet_name_clean, index=False)
        
        # Create a summary sheet
        summary_data = []
        for sheet_name, df_step in step_dataframes.items():
            if sheet_name != 'Original_Data':
                summary_data.append({
                    'Step': sheet_name,
                    'Companies_Remaining': len(df_step),
                    'Companies_Filtered_Out': len(df) - len(df_step) if sheet_name != 'Original_Data' else 0,
                    'Percentage_Remaining': f"{len(df_step)/len(df)*100:.1f}%"
                })
        
        summary_df = pd.DataFrame(summary_data)
        summary_df.to_excel(writer, sheet_name='Summary', index=False)
        print(f"💾 Writing sheet: Summary")
    
    print(f"\n✅ All results saved to single workbook: {final_workbook}")
    print(f"📊 Workbook contains {len(step_dataframes) + 1} sheets:")
    for sheet_name in step_dataframes.keys():
        sheet_display = sheet_name[:31] if len(sheet_name) > 31 else sheet_name
